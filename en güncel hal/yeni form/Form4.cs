...
namespace yeni_form
{
    public partial class Form4 : Form
    {
        ...
        {  
                // girilen kart numarasının geçerli olup olmadığı tespit edilr  kart no geçerli is eödeme alınır 
            if (Kredi_Kart_Lunh_Algoritmasi(textBox1.Text) == true)
                MessageBox.Show ("Kart No Geçerli ; ödeme alındı ") ;
            else
                //kart no geçersiz ise ödeme alınmaz 
                MessageBox.Show("Kart No Geçersiz") ;
            foreach (Control item in this.Controls)
            {
                if (item is TextBox)
                {  // ödeme alındıktan sonra ve hatalı giriş yapıldıktan sonra girilen bilgiler temizlenir 
